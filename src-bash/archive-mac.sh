...
OPTS=$(ggetopt --name $(basename $0) --options u:o:s:v:d: --longoptions url:,output:,script:,scriptversion:,scriptsdirectory: -- $@)
if [[ $? -ne 0 ]]; then
    exit 2
fi
echo OPTS=["$OPTS"]
...
while true;do
  case "$1" in
    ...
    -v|--scriptversion)
      ...
      ;;
    -d|--scriptsdirectory)
      scriptsdirectory="--volume $(greadlink -f -- $2):/resources/scripts/:ro"
      shift 2
      ;;
    --)
      break
